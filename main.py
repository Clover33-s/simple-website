...
def main():
    ...
    # Step 1: Pre-authenticate with YouTube to prevent wasting API credits
    log.info("Performing pre-authentication check with YouTube...")
    uploader = YouTubeUploader()
    if not uploader.get_authenticated_service():
        log.error("YouTube authentication failed. Please check your credentials and setup. Exiting.")
        return
    log.info("YouTube authentication successful. Proceeding with content generation.")

    # Step 2: Generate prompts using the LLM
    prompts = []
    ...
    # Step 3: Generate images from the prompts using Stability AI
    media_paths = []
    ...
    # Step 4: Compile the video and upload it
    if media_paths:
        ...
        if os.path.exists(output_video_path):
            ...
            # The uploader instance is already authenticated from the pre-auth check
